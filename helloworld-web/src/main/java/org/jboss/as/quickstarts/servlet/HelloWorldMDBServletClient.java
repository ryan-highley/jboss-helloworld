 ...
 * @JMSDestinationDefinition( name = "java:jboss/jms/ivt/IVTQueue", interfaceName =
 * "javax.jms.Queue", destinationName = "HelloWorldMDBQueue" ),
 * @JMSDestinationDefinition( name = "java:/topic/HELLOWORLDMDBTopic", interfaceName =
 * "javax.jms.Topic", destinationName = "HelloWorldMDBTopic" ) })
 */
/**
 * <p>
...
public class HelloWorldMDBServletClient extends HttpServlet {
  ...
  @Inject
  @JMSConnectionFactory("java:/jms/ivt/IVTCF")
  private JMSContext context;

  @Inject
  @JMSConnectionFactory("java:/jms/ivt/IVTCF2")
  private JMSContext context2;

  @Resource(lookup = "java:/jms/ivt/IVTQueue")
  private Queue queue;

  @Resource(lookup = "java:/jms/ivt/IVTQueue2")
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    LOGGER.info("Handling doGet: {}", req);
    ...
    try {
      boolean useSecondQueueMgr = req.getParameterMap().keySet().contains("qm2");
      final Destination destination = useSecondQueueMgr ? queue2 : queue;
      @SuppressWarnings("resource")
      final JMSContext destContext = useSecondQueueMgr ? context2 : context;

      out.write("<p>Sending messages to <em>" + destination + "</em></p>");
      out.write("<h2>Following messages will be send to the destination:</h2>");
      for (int i = 0; i < MSG_COUNT; i++) {
        String text = "This is message " + (i + 1);
        destContext.createProducer().send(destination, text);
